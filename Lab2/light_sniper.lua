...
function isObstacleInFront(arr)
	...
	if arr[1].value > proximity_threshold and arr[24].value > proximity_threshold then return true end return false
end
function isObstacleOnTheRight(arr)
	...
	for i = 18, 19 do
		log(arr[i].value)
		if arr[i].value > proximity_threshold then
		    free = true
		end
    end	
   	return free
...
function turnLeft()
	robot.wheels.set_velocity(-110, 110)
end
function turnRight()
	robot.wheels.set_velocity(110, -110)
end
...
--[[	n_steps = n_steps + 1 ]]
	...
	max_valuep, max_idxp = findMaxValue2(robot.proximity)
		if not find_obstacle and max_idxl > 0 and robot.proximity[max_idxl].value <= 0.1 then
			...
			robot.wheels.set_velocity(left_v, right_v)
		elseif isObstacleInFront(robot.proximity) then
